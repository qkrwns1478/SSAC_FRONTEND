  ...
  try {
    ...
    return (
      <section className="mb-12">
        <h2 className="mb-4 text-2xl font-bold text-gray-900">신규 콘텐츠</h2>
          ...
          {items.map((item) => (
            <Link key={item.id} href={`/content/${item.id}`}>
              <article className="h-full rounded-xl border border-gray-200 bg-white p-5 shadow-sm transition-shadow hover:shadow-md">
                ...
                <h3 className="mb-3 line-clamp-2 font-semibold text-gray-900">{item.title}</h3>
                <p className="text-sm text-gray-500">{formatDate(item.registeredAt)} 등록</p>
              </article>
            </Link>
          ))}
        </div>
